-- ==================================================================
-- RSA FFM Hub — SETUP TRỌN BỘ database Supabase
-- Project MỚI: dán TOÀN BỘ file này vào SQL Editor -> Run (1 lần duy nhất).
-- Project CŨ (đã chạy 0001+0002 của bản Next.js): chỉ chạy PHẦN 3/3.
-- ==================================================================
...
create type perm_scope_t      as enum ('none','own','all');   -- phạm vi mỗi quyền: không / chỉ của mình / toàn bộ
...
create index on order_items (factory_id);
create index on order_items (item_status);
create index on order_items (tracking_status);
create index on topups (factory_id);
...
create view v_factory_balance as
...
create index on activity_log (actor);
