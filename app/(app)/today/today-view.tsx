...
import { updateOrderItem } from "../orders/[id]/actions";
...
export function TodayView({ items, today, isFFM }: { items: TodayItem[]; today: string; isFFM: boolean }) {
  const [list, setList] = useState(items);
  const [q, setQ] = useState("");
  const [seller, setSeller] = useState("");
  const [busy, setBusy] = useState<string | null>(null);

  const sellers = useMemo(() => [...new Set(items.map((i) => i.seller).filter(Boolean) as string[])].sort(), [items]);

  async function setStatus(id: string, item_status: string) {
    setBusy(id);
    const r = await updateOrderItem(id, { item_status });
    setBusy(null);
    if (r.ok) setList((l) => l.map((x) => (x.id === id ? { ...x, itemStatus: item_status } : x)));
  ...
  const overdue = useMemo(() => filtered.filter((i) => overdueDays(i, today) != null).sort((a, b) => (a.deadline ?? "").localeCompare(b.deadline ?? "")), [filtered, today]);
  ...
  function List({ rows, mode }: { rows: TodayItem[]; mode: "overdue" | "design" | "push" }) {
    ...
    return (
      <>
            ...
            {shown.map((r) => {
              ...
              return (
                <tr key={r.id} className="border-t border-slate-100 first:border-0 hover:bg-slate-50">
                  ...
                  <td className="max-w-[130px] truncate px-2 py-2 text-slate-500">{r.customerName ?? "—"}</td>
                  <td className="max-w-[240px] truncate px-2 py-2">{r.productTitle ?? "—"}{r.size ? ` · ${r.size}` : ""}</td>
                  ...
                  <td className="whitespace-nowrap px-3 py-2 text-right">
                    {busy === r.id ? <span className="text-xs text-slate-400">…</span> : isFFM ? (
                      <select value={r.itemStatus} onChange={(e) => setStatus(r.id, e.target.value)}
        ...
        {rows.length > shown.length && (
          <div className="border-t border-slate-100 px-3 py-2 text-center text-xs text-slate-400">Còn {rows.length - shown.length} việc nữa — lọc bớt để xem.</div>
        )}
      </>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm: Order ID, khách, sản phẩm, size…"
          className="w-full max-w-sm rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
        <select value={seller} onChange={(e) => setSeller(e.target.value)} className="rounded-md border border-slate-300 px-2 py-1.5 text-sm">
        ...
        {isFFM && <span className="text-xs text-slate-400">· FFM: đổi trạng thái ngay ở cột phải</span>}
      </div>

      <Card title="⏰ Quá hạn ship" hint="trễ nhất lên đầu — xử lý ngay" count={overdue.length} tone="bg-red-50 text-red-800"><List rows={overdue} mode="overdue" /></Card>
      <Card title="🎨 Chờ design" hint="đơn để lâu nhất lên đầu" count={design.length} tone="bg-orange-50 text-orange-800"><List rows={design} mode="design" /></Card>
      <Card title="🏭 Chờ FFM đẩy xưởng" hint="đủ điều kiện, chờ đẩy" count={push.length} tone="bg-amber-50 text-amber-800"><List rows={push} mode="push" /></Card>
    </div>
  );
}
